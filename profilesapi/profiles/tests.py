...
class RegistrationTestCase(APITestCase):
    # APITestCase provides a client to make these requests to the endpoints.
    def test_registration(self):
        data = {"username": "testcase", "email": "test@test.com",
                "password1": "fake1234", "password2": "fake1234"}
        # generate a response for the endpoint using the client from
        # APITestCase
        response = self.client.post("/api/rest-auth/registration/", data)
        # check it's what you want it to be
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)


class ProfileViewSetTestCase(APITestCase):
    # first methods create a new user object to check authentication
    list_url = reverse("profile-list")

    def setUp(self):
        self.user = User.objects.create_user(
            username="fake", password="fake1234")
    ...
    def test_profile_detail_retrieve(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["user"], 'fake')

    def test_profile_update_by_owner(self):
        response = self.client.put(reverse(
            "profile-detail", kwargs={"pk": 1}),
            {"city": "brooklyn",
             "bio": "not a real bio 1234"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        print(json.loads(response.content))
        print()
        self.assertEqual(json.loads(response.content),
                         {"id": 1, "user": "fake", "city": "brooklyn", "bio": "not a real bio 1234", "avatar": None})
    ...
    def test_profile_update_by_random_user(self):
        ...
        response = self.client.put(reverse(
            "profile-detail", kwargs={"pk": 1}), {"bio": "hacked!!"})
...
class ProfileStatusViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.status = ProfileStatus.objects.create(
            user_profile=self.user.profile, status_content="status test 1234")
        self.token = Token.objects.create(user=self.user)
        self.api_authentication()
    ...
    def test_status_create(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data["user_profile"], "fake")
    ...
    def test_status_update_random_user(self):
        random_user = User.objects.create_user(username="random",
                                               password="password123")
        self.client.force_authenticate(user=random_user)
        data = {"status_content": "You've been hacked!"}
        response = self.client.put(reverse(
            "status-detail", kwargs={"pk": 1}), data=data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
